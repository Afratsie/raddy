---
title: 指标
description: raddy 在 --metrics-addr 监听器上暴露的 Prometheus 指标。
---

用 `--metrics-addr` 启动 raddy,即可通过 HTTP 暴露 Prometheus 指标:
...
| 指标 | 类型 | 说明 |
|---|---|---|
| `raddy_requests_total` | Counter | raddy 服务的 HTTP 请求总数(QPS 来源) |
| `raddy_request_duration_seconds` | Histogram | HTTP 请求耗时(秒) |
